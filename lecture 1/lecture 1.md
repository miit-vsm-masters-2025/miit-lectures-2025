...
- План семестра
  ...
  - По вторникам две пары, на них будет больше теории и кодинга в моем исполнении.
  - По субботам - практика, где код писать будете уже вы.
    - В планах - попробовать набросать три относительно приближенных к реальной жизни проекта.
...
- Управление памятью
  ...
  - Reference counting
    ...
    - Кто может сказать, какие у нее есть проблемы?
      - А что если есть циклические ссылки?
        ...
        - **Пример**: Reference counting и циклические ссылки в Python
        - **Пример**: Слабые ссылки для разрыва циклов
  ...
  - Экспериментальные техники управления памятью: Rust.
    ...
    - **Пример**: Rust: borrow checker
    - **Пример**: Rust: передача данных между потоками безопасно
- Микросервисы. Почему конкретный язык программирования уже не так важен в современном мире.
  - Микросервисы - достаточно маленькие по (количеству строк кода) сервисы. Обычно не больше 5-10 тысяч строк кода.
  ...
  - **Пример**: Микросервисы: минимальный REST (FastAPI)
  - **Пример**: Микросервисы: gRPC — определение контракта и сервер (Python)
